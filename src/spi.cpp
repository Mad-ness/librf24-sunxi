...
SPI::SPI(string spidev, uint32_t speed, int bits):fd(-1), _spi_speed(RF24_SPIDEV_SPEED) {
    this->_spi_speed = speed;
    this->spidev = spidev;
    this->bits = bits;
    begin(0, this->_spi_speed);
}
...
void SPI::begin(int busNo,uint32_t spi_speed){
    ...
    printf("Opening the SPI device: %s\n", this->spidev.c_str());

    this->fd = open(this->spidev.c_str(), O_RDWR);
